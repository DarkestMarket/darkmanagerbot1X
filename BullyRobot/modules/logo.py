...
from BullyRobot import OWNER_ID
from BullyRobot.events import register
from BullyRobot import telethn
...
async def lego(event):
 ...
 try:
    ...
    fillcolor = "black"
    shadowcolor = "blue"
    fnt = glob.glob("./BullyRobot/resources/fonts/*")
    ...
    fname = "fallen.png"
    img.save(fname, "png")
    await telethn.send_file(event.chat_id, file=fname, caption = f"ʟᴏɢᴏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ Dark Controller")         
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'ғʟᴏᴏᴅᴡᴀɪᴛ ᴇʀʀᴏʀ, ʀᴇᴩᴏʀᴛ ᴛʜɪs ᴀᴛ @DarkestMarket666')


__mod_name__ = "Lᴏɢᴏ​"

__help__ = """
Dark Controller can create some beautiful and attractive logo for your profile pics.
